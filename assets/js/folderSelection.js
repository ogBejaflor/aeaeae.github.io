...
function attachWindowContentSelectionHandlers() {
  document.querySelectorAll('.window .window-content').forEach(content => {
    if (content.classList.contains('audio-app-container')) return;
...
function onDragFolder(event) {
  ...
  let isSelfScope = false;
  if (topWin) {
    isSelfScope = draggedGroup.some(el => el.getAttribute('data-window') === topWin.id);
  }

  // If scope changes during drag, re-parent whole group into new scope and freeze it
  if (topScope && topScope !== groupScopeEl && !isSelfScope) {
...
function stopDragFolder(e) {
  ...
  if (targetFolder) {
    ...
    const isSelfDrop = draggedGroup.some(el => el.getAttribute('data-window') === windowId);
    
    const targetWindow = !isSelfDrop ? document.getElementById(windowId) : null;
